...
function runFrame(now: number, refs: RunFrameRefs) {
  ...
  rafRef.current = settled ? null : requestAnimationFrame((t) => runFrame(t, refs));
}
...
export default function OptionWheel({
  ...
  const startLoop = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame((now) =>
      runFrame(now, { itemRefs, rafRef, lastRef, posRef, targetRef, cfgRef }),
    );
  }, []);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- applyTarget itself isn't a dependency: it's re-created only when startLoop/playTick change, neither of which should re-trigger this re-layout.
  ...
  if (items.length === 0) {
    return (
      <div
        className={`option-wheel option-wheel--empty${side === "right" ? " option-wheel--right" : ""}`}
        style={{ "--ow-inset": `${inset}px` } as CSSProperties}
      >
        <p className="option-wheel__empty-text">Study a topic to start unlocking related ones here.</p>
      </div>
    );
  }
